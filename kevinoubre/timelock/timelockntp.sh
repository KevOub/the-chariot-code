...
########################### 
#   THIS CODE IS GOOD FOR WHEN THE CURRENT TIME IS NOT NEEDED AND 
#   AN EPOCH IS GIVEN ALONG WITH A CENTRAL TIME SERVER (HENCE, NTP)
########################### 
#echo $(nc $SERVER $PORT)
...
#CURRENTTIME=$2
# CURRENTTIME=$2
CURRENTTIME=$( date --date="$(nc $SERVER $PORT)" )
#CURRENTTIME=$(date --date="Fri May  7 08:18:56 AM CDT 2021" +%T)
...
# ymd_epoch=$(echo $CURRENTTIME | tr -d " " | cut -c 1-8) # not needed with provided datetime defaulting to seconds
ymd_given=$(echo $EPOCHTIMEGIVEN | tr -d " " | cut -c 1-8)

# make hte hours minutes seconds in the form HH:MM:SS
# hms_epoch=$(echo $CURRENTTIME | tr -s " " ":" | cut -c 12-) # defaults to seconds now
...
if [ $DEBUG == true ]; then
    ...
    echo "current time is =   " $( date --date="$CURRENTTIME" +'%Y %m %d %H %M %S' ) 
    echo "epoch time is =   " $EPOCHTIMEGIVEN
fi
...
# echo $alpha$numeric${HASH:16:1} # uncomment here to get the 16th digit to be included again
